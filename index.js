...
function getNumberFromString(s) {
  ...
  return isNaN(result) ? "Couldn't extract a number from the provided string." : result;
...
function getIntegerFromString(s) {
  ...
  // Перевіряємо, чи є отримане значення числом за допомогою isNaN
  return isNaN(result) ? "Couldn't extract an integer from the provided string." : result;
...
function isValueNaN(value) {
  ...
  // Якщо значення не є NaN, повертаємо текст  "The value is NaN.".
}
